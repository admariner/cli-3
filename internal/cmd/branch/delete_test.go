...
func TestBranch_DeleteCmd_WithDeleteDescendants(t *testing.T) {
	c := qt.New(t)
	...
	svc := &mock.DatabaseBranchesService{
		DeleteFn: func(ctx context.Context, req *ps.DeleteDatabaseBranchRequest) error {
			...
			c.Assert(req.Database, qt.Equals, db)
			c.Assert(req.Organization, qt.Equals, org)
			c.Assert(req.DeleteDescendants, qt.IsTrue)
			return nil
		},
	}
	...
	cmd := DeleteCmd(ch)
	cmd.SetArgs([]string{db, branch, "--force", "--delete-descendants"})
	err := cmd.Execute()
...
func TestBranch_DeleteCmd_PostgreSQL_WithDeleteDescendants(t *testing.T) {
	c := qt.New(t)
