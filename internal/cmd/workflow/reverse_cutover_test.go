...
func TestWorkflow_ReverseCutoverCmd(t *testing.T) {
	c := qt.New(t)
	...
	expectedWorkflow := &ps.Workflow{
		...
		Name:              "test-workflow",
		State:             "cutover_reversed",
		CreatedAt:         createdAt,
		UpdatedAt:         createdAt,
		PrimariesSwitched: false,
		ReplicasSwitched:  false,
	...
	svc := &mock.WorkflowsService{
		ReverseCutoverFn: func(ctx context.Context, req *ps.ReverseCutoverWorkflowRequest) (*ps.Workflow, error) {
...
func TestWorkflow_ReverseCutoverCmd_Error(t *testing.T) {
	c := qt.New(t)
