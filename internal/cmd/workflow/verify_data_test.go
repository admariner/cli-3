...
func TestWorkflow_VerifyDataCmd(t *testing.T) {
	c := qt.New(t)
	...
	expectedWorkflow := &ps.Workflow{
		...
		VerifyDataAt: &createdAt,
	}

	// Mock the workflow service
	svc := &mock.WorkflowsService{
		VerifyDataFn: func(ctx context.Context, req *ps.VerifyDataWorkflowRequest) (*ps.Workflow, error) {
...
func TestWorkflow_VerifyDataCmd_Error(t *testing.T) {
	c := qt.New(t)
