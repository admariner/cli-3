...
func TestKeyspace_ResizeCmdOnlyClusterSize(t *testing.T) {
	c := qt.New(t)
...
func TestKeyspace_ResizeCmdOnlyReplicas(t *testing.T) {
	c := qt.New(t)
	...
	cmd := ResizeCmd(ch)
	cmd.SetArgs([]string{db, branch, keyspace, "--additional-replicas", "3"})
	err := cmd.Execute()
	c.Assert(err, qt.IsNil)
	c.Assert(svc.ResizeFnInvoked, qt.IsTrue)
	c.Assert(buf.String(), qt.JSONEquals, krr)
